...
The other essential component of the dataset is about SDP and ETS. Firstly, we would like to report the statistics of urine cotimean levels in mothers and children at different time points. The average urine cotimean of mothers at week 34 gestation is 49.75, with standard deviation 97.69. The average urine cotimean of mothers at 6 months postpartum is 100.49, with standard deviation 179.34. The average urine cotimean of children at 6 months postpartum is 4.04, with standard deviation 7.62. 

However, for the primary exposure of interest, cotimean levels at two time points are far from enough. As stated in **2.2**, we also created summary variables for both both prenatal and postnatal smoke
exposure, `SDP` and `ETS`, and their corresponding intensity, `SDP_intensity` and `ETS_intensity`. 24 subjects report no smoking during pregnancy (`SDP`=0), and 14 admit that they have smoked during pregnancy(`SDP`=1). The average intensity of SDP is 0.8919 with standard deviation equals 1.33. As for postnatal smoke exposure `ETS`, only 12 subjects report no environmental tobacco (`ETS`=0), and 28 report some level of tobacco exposure when the children are at early age (`ETS`=1).

```{r include=FALSE}
table(df$SDP)
table(df$ETS)
sd(df$SDP_intensity, na.rm=T)
summary(df$ETS_intensity)
```
...
ggscatter(df, x = "SDP_intensity", y = "ETS_intensity", point = FALSE,
